...
class build_transformer(nn.Module):
    def __init__(self, opt,num_classes,block = 4 ,return_f=False):
        ...
        if opt.backbone == "VIT-S":
            ...
            model_path = opt.pretrain_path
            # Vision Mamba v2 (更强的版本)
            transformer_name = "vision_mamba_v2_small_patch16_224_FSRA"
            self.in_planes = 768

            print('using Transformer_type: {} as a backbone'.format(transformer_name))

            self.transformer = vision_mamba_v2_small_patch16_224_FSRA(img_size=(256,256), stride_size=[16, 16], 
                                                                     drop_rate=0.0, local_feature=False)
            if model_path and model_path != '':
                self.transformer.load_param(model_path)
            else:
